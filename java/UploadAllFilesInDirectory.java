 ...
 * </pre>
 * <p>
 * The script will loop over the whole directory (and subdirectories), find all relevant files
...
class UploadAllFilesInDirectory {
  ...
  private static void uploadDirectory(Arguments arguments) throws IOException {
    Files.walkFileTree(Paths.get(arguments.inputDirectory), new FileVisitor<Path>() {
      @Override
  ...
  private static void uploadFile(Path file, Arguments arguments)
      throws IOException, InterruptedException {
    String fileName = file.toFile().getName();
    HttpRequest uploadDataRequest = newAuthenticatedMultipartFormRequest(arguments)
        .uri(
            toAbsoluteURI(String.format("/datasets/%s/data/upload", arguments.dataSetId)))
  ...
  private static List<byte[]> buildMultipartData(Map<String, byte[]> data,
    ...
    var separator = ("--" + UploadAllFilesInDirectory.MULTI_PART_BOUNDARY +
    ...
    byteArrays.add(("--" + UploadAllFilesInDirectory.MULTI_PART_BOUNDARY + "--")
        .getBytes(StandardCharsets.UTF_8));
    return byteArrays;
  }
